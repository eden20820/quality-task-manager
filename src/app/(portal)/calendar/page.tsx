...
export default async function CalendarPage() {
  ...
  const [{ data: tasks, error: tasksError }, { data: reminders, error: remindersError }, { data: calibrations }, { data: followups }, { data: calibrationSettings }] = await Promise.all([
  ...
  return <CalendarView tasks={(tasks ?? []) as CalendarTask[]} reminders={(reminders ?? []) as Reminder[]} calibrations={(calibrationSettings?.calibration_alerts_enabled === false ? [] : (calibrations ?? [])) as CalendarCalibration[]} followups={(followups ?? []) as CalendarFollowup[]} />;
}
