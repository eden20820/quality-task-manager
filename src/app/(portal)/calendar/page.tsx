...
export default async function CalendarPage() {
  ...
  const [{ data: tasks, error: tasksError }, { data: reminders, error: remindersError }, { data: calibrations }, { data: followups }] = await Promise.all([
    ...
    supabase.from("quality_followups").select("id, category, reference_number, name, quantity, opened_at, created_at").eq("status", "open").eq("alerts_enabled", true).order("created_at"),
  ]);
