...
export function SuppliersManager({ rows, alertsEnabled }: { rows: SupplierRow[]; alertsEnabled: boolean }) {
  ...
  return <div className="space-y-7">
    ...
    <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
      ...
      {visible.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1860px] text-right text-sm"><thead className="sticky top-0 bg-slate-50"><tr>{["#", "שם הספק", "מוצר / שירות", "הסמכה", "ניסיון", "סטטוס", "סוג הסמכה", "תאריך תפוגה", "מצב תוקף", "עמידה בלו״ז 30%", "איכות 30%", "מקצועיות 15%", "עמידה בדרישות 25%", "הערכה משוקללת", "הערות", "פעולות"].map((heading) => <th key={heading} className={`whitespace-nowrap p-4 ${heading === "פעולות" ? "sticky left-0 z-10 border-r bg-slate-50" : ""}`}>{heading}</th>)}</tr></thead><tbody>{visible.map((row) => { const status = expiryStatus(row.expiration_date); return <tr key={row.id} onDoubleClick={() => { setEditing(row); setEditMessage(""); }} className="group cursor-pointer border-t align-top hover:bg-slate-50"><td className="p-4 font-bold">{row.supplier_number || "—"}</td><td className="max-w-64 p-4 font-bold">{row.supplier_name}</td><td className="max-w-56 p-4">{row.product_service || "—"}</td><td className="p-4">{row.has_certification ? "כן" : "לא"}</td><td className="p-4">{row.has_experience ? "כן" : "לא"}</td><td className="p-4">{row.status === "Approved" ? "מאושר" : row.status}</td><td className="max-w-64 p-4">{row.certification_type || "—"}</td><td className="whitespace-nowrap p-4 font-bold">{formatDate(row.expiration_date)}</td><td className="p-4"><span className={`whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-bold ${status.color}`}>{status.text}</span></td><td className="p-4 text-center">{row.delivery_score ?? "—"}</td><td className="p-4 text-center">{row.quality_score ?? "—"}</td><td className="p-4 text-center">{row.professionalism_score ?? "—"}</td><td className="p-4 text-center">{row.requirements_score ?? "—"}</td><td className="p-4 text-center text-base font-extrabold">{row.weighted_score ?? "—"}</td><td className="max-w-72 p-4">{row.notes || "—"}</td><td onDoubleClick={(event) => event.stopPropagation()} className="sticky left-0 border-r bg-white p-4 group-hover:bg-slate-50"><div className="flex items-center gap-2"><button type="button" onClick={() => { setEditing(row); setEditMessage(""); }} className="inline-flex h-9 items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 font-bold text-blue-700 transition hover:bg-blue-100"><Pencil className="h-4 w-4" />עריכה</button><form action={deleteSupplier.bind(null, row.id)}><button aria-label={`מחיקת הספק ${row.supplier_name}`} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-400 transition hover:bg-red-50 hover:text-red-600"><Trash2 className="h-5 w-5" /></button></form></div></td></tr>; })}</tbody></table></div> : <p className="p-10 text-center text-slate-500">אין ספקים להצגה בסינון זה</p>}
    </section>
