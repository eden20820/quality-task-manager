...
export function DashboardTasks({ tasks }: { tasks: DashboardTask[] }) {
  ...
  return <Card>
    ...
    <CardContent>{tasks.length ? <div className={view === "cards" ? "grid items-start gap-3 sm:grid-cols-2 xl:grid-cols-3" : "divide-y overflow-hidden rounded-xl border"}>{tasks.map((task) => <button key={task.id} type="button" onClick={() => setSelected(task)} className={`min-w-0 text-right transition hover:bg-slate-50 ${view === "cards" ? "rounded-xl border bg-white p-4 shadow-sm hover:border-slate-300 hover:shadow-md" : "block w-full px-4 py-4"}`}>
      <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-extrabold text-slate-950">{task.title}</p><p className="mt-0.5 text-xs font-bold text-slate-400">משימה #{task.task_number}</p></div><span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${task.priority === "urgent" ? "bg-red-500" : task.priority === "high" ? "bg-orange-400" : "bg-blue-400"}`} /></div>
      <div className="mt-3 flex flex-wrap gap-1.5"><Badge variant="secondary">{statusLabels[task.status] ?? task.status}</Badge><Badge variant="outline">עדיפות: {priorityLabels[task.priority] ?? task.priority}</Badge></div>
      <div className="mt-3 space-y-3 text-sm leading-6"><div><span className="font-bold text-slate-500">תיאור</span><p className="whitespace-pre-wrap break-words text-slate-700">{task.description || "אין תיאור"}</p></div><div><span className="font-bold text-slate-500">עדכון סטטוס</span><p className="whitespace-pre-wrap break-words text-slate-700">{task.status_note || "אין עדכון סטטוס"}</p></div></div>
      <dl className="mt-3 grid gap-2 border-t pt-3 text-xs text-slate-500 sm:grid-cols-3"><div><dt className="font-bold">אחראים</dt><dd className="mt-0.5 break-words text-slate-700">{formatAssignees(task.assignees)}</dd></div><div><dt className="font-bold">תאריך יעד</dt><dd className="mt-0.5 text-slate-700">{formatDate(task.due_date)}</dd></div><div><dt className="font-bold">נוצרה</dt><dd className="mt-0.5 text-slate-700">{formatDate(task.created_at)}</dd></div></dl>
