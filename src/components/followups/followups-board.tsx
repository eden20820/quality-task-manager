...
export function FollowupsBoard({ rows }: { rows: Followup[] }) {
  return <div className="space-y-7"><div><h1 className="text-3xl font-extrabold sm:text-4xl">פק״ע, אי התאמה, ECO</h1><p className="mt-2 text-slate-500">מעקב אחר רשומות פתוחות וסגורות. רשומה שלא נסגרה בתוך שבוע תופיע ביומן ותישלח במייל לעדן ולסרגיי.</p></div><div className="grid items-start gap-5 xl:grid-cols-3">{(["pka", "nonconformity", "eco"] as const).map((category) => <Column key={category} category={category} rows={rows.filter((row) => row.category === category)} />)}</div></div>;
}
