...
export type Followup = { id: string; category: "pka" | "nonconformity" | "eco"; reference_number: string; name: string | null; quantity: number | null; status: "open" | "closed"; opened_at: string; created_at: string; notes: string | null };
...
function Column({ category, rows }: { category: Followup["category"]; rows: Followup[] }) {
  ...
  return <section className="rounded-2xl border bg-white shadow-sm">
    ...
    <form action={action} className="space-y-3 border-b bg-slate-50 p-4"><input type="hidden" name="category" value={category} /><input required name="reference_number" placeholder={`מספר ${labels[category]}`} className="h-11 w-full rounded-lg border bg-white px-3" /><input required name="name" placeholder="שם" className="h-11 w-full rounded-lg border bg-white px-3" />{category === "pka" && <input required name="quantity" type="number" min="0" step="1" placeholder="כמות" className="h-11 w-full rounded-lg border bg-white px-3" />}<input required name="opened_at" type="date" defaultValue={today()} className="h-11 w-full rounded-lg border bg-white px-3" /><select name="status" defaultValue="open" className="h-11 w-full rounded-lg border bg-white px-3 font-bold"><option value="open">נפתח</option><option value="closed">נסגר</option></select><input name="notes" placeholder="הערה (לא חובה)" className="h-11 w-full rounded-lg border bg-white px-3" /><button disabled={pending} className="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-slate-950 font-bold text-white"><Plus className="h-4 w-4" />הוספה</button>{state.message && <p className="text-sm font-bold">{state.message}</p>}</form>
    <div className="space-y-3 p-4">{rows.map((row) => { const overdue = row.status === "open" && dueDate(row.created_at) < new Date(); return <article key={row.id} className={`rounded-xl border p-4 ${overdue ? "border-red-200 bg-red-50" : "bg-white"}`}><div className="flex items-start justify-between gap-2"><div><p className="font-extrabold">{row.reference_number}</p>{row.name && <p className="mt-1 font-semibold text-slate-700">{row.name}</p>}{row.category === "pka" && row.quantity !== null && <p className="mt-1 text-sm text-slate-600">כמות: {row.quantity}</p>}<p className="mt-1 text-xs text-slate-500">תאריך הרשומה: {new Intl.DateTimeFormat("he-IL").format(new Date(`${row.opened_at}T12:00:00`))}</p>{row.status === "open" && <p className={`mt-1 text-xs font-bold ${overdue ? "text-red-600" : "text-slate-500"}`}>התראה ראשונה: {new Intl.DateTimeFormat("he-IL").format(dueDate(row.created_at))}</p>}{row.notes && <p className="mt-2 text-sm">{row.notes}</p>}</div><form action={deleteFollowup.bind(null, row.id)}><button aria-label="מחיקה" className="text-slate-400 hover:text-red-600"><Trash2 className="h-4 w-4" /></button></form></div><form action={toggleFollowup.bind(null, row.id, row.status === "open" ? "closed" : "open")}><button className={`mt-3 flex w-full items-center justify-center gap-2 rounded-lg border py-2 text-sm font-bold ${row.status === "closed" ? "bg-emerald-50 text-emerald-700" : "bg-white"}`}>{row.status === "closed" ? <CheckCircle2 className="h-4 w-4" /> : <CircleDot className="h-4 w-4" />}{row.status === "closed" ? "נסגרה — פתיחה מחדש" : "פתוחה — סימון כסגורה"}</button></form></article>; })}{!rows.length && <p className="py-5 text-center text-sm text-slate-500">אין רשומות</p>}</div>
  </section>;
}
