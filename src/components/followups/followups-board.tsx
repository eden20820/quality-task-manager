...
function EditFollowupDialog({ row }: { row: Followup }) {
  ...
  return <><button type="button" onClick={() => setOpen(true)} title="עריכת כל השדות" aria-label={`עריכת ${row.reference_number}`} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-sky-200 bg-sky-50 text-sky-700 transition hover:bg-sky-100"><Pencil className="h-4 w-4" /></button>{open ? <div className="fixed inset-0 z-50 flex items-center justify-center p-3" role="dialog" aria-modal="true" aria-label={`עריכת ${row.reference_number}`}><button type="button" aria-label="סגירה" className="absolute inset-0 bg-slate-950/60" onClick={() => !pending && setOpen(false)} /><form action={save} className="relative z-10 max-h-[90vh] w-full max-w-3xl overflow-auto rounded-2xl bg-white p-6 shadow-2xl"><input type="hidden" name="category" value={row.category} /><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-extrabold">עריכת {labels[row.category]}</h2><p className="text-sm text-slate-500">ניתן לערוך את כל שדות הרשומה</p></div><button type="button" onClick={() => setOpen(false)} aria-label="סגירה" className="rounded-lg p-2"><X className="h-5 w-5" /></button></div><div className="grid gap-4 sm:grid-cols-2">
...
function FollowupsTable({ category, rows }: { category: Category; rows: Followup[] }) {
  ...
  return <div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="max-h-[72vh] overflow-auto"><table className={`h-fit w-full table-auto border-collapse text-right text-xs leading-4 ${category === "eco" ? "min-w-[1780px]" : category === "nonconformity" ? "nonconformity-table min-w-[1780px] md:min-w-0" : "min-w-[1120px]"}`}>
    ...
    <tbody className="divide-y divide-slate-100 align-top">{rows.map((row) => { const alertDate = dueDate(row.created_at); const overdue = row.alerts_enabled && row.status !== "closed" && alertDate < new Date(); return <tr key={row.id} className={`h-auto align-top transition-colors hover:bg-slate-50 ${overdue ? "bg-red-50/70" : ""}`}>
      ...
      <td className="px-3 py-2"><div className="flex items-center justify-center gap-1.5"><EditFollowupDialog row={row} /><form action={toggleFollowup.bind(null, row.id, row.status === "closed" ? "open" : "closed")}><button title={row.status === "closed" ? "פתיחה מחדש" : "סימון כסגורה"} aria-label={row.status === "closed" ? "פתיחה מחדש" : "סימון כסגורה"} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-emerald-200 bg-emerald-50 text-emerald-700 transition hover:bg-emerald-100">{row.status === "closed" ? <CircleDot className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}</button></form><form action={toggleFollowupAlerts.bind(null, row.id, !row.alerts_enabled)}><button title={row.alerts_enabled ? "כיבוי התראות" : "הפעלת התראות"} aria-label={row.alerts_enabled ? "כיבוי התראות" : "הפעלת התראות"} className={`inline-flex h-8 w-8 items-center justify-center rounded-md border transition ${row.alerts_enabled ? "border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100" : "border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100"}`}>{row.alerts_enabled ? <Bell className="h-4 w-4" /> : <BellOff className="h-4 w-4" />}</button></form><form action={deleteFollowup.bind(null, row.id)}><button title="מחיקה" aria-label="מחיקה" className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100"><Trash2 className="h-4 w-4" /></button></form></div></td>
    </tr>; })}</tbody>
  </table></div></div>;
}
...
function FollowupDetailsDialog({ row, onClose }: { row: Followup; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-labelledby={`details-${row.id}`}><button type="button" aria-label="סגירת פרטי הרשומה" className="absolute inset-0 bg-slate-950/45" onClick={onClose} /><section dir="rtl" className="absolute inset-y-0 left-0 flex h-dvh w-full max-w-lg flex-col bg-white shadow-2xl"><header className="flex shrink-0 items-start justify-between gap-4 border-b p-5"><div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{labels[row.category]}</span><h2 id={`details-${row.id}`} className="mt-3 text-xl font-extrabold leading-7">{recordTitle(row)}</h2><p className="mt-1 text-sm font-bold text-slate-500">{row.reference_number}</p></div><button type="button" onClick={onClose} aria-label="סגירה" className="rounded-lg border p-2 text-slate-500 hover:bg-slate-100"><X className="h-5 w-5" /></button></header><div className="min-h-0 flex-1 overflow-y-auto p-5"><dl className="grid gap-3 sm:grid-cols-2">{fields.map(([label, value]) => <div key={label} className="rounded-xl border bg-slate-50 p-3"><dt className="text-xs font-bold text-slate-500">{label}</dt><dd className="mt-1 whitespace-pre-wrap break-words text-sm font-semibold text-slate-800">{value}</dd></div>)}</dl><div className="mt-4 rounded-xl border p-4"><h3 className="text-sm font-extrabold">הערות</h3><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-7 text-slate-700">{row.notes || "אין הערות"}</p></div></div><footer className="flex shrink-0 items-center justify-between border-t bg-slate-50 p-4"><span className="text-xs text-slate-500">כל פרטי הרשומה מוצגים כאן</span><EditFollowupDialog row={row} /></footer></section></div>;
}
...
function FollowupCard({ row }: { row: Followup }) {
  ...
  return <><article className={`group flex min-h-44 cursor-pointer flex-col rounded-xl border bg-white p-3.5 text-right shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md ${overdue ? "border-red-200 bg-red-50/40" : "border-slate-200"}`} onClick={() => setDetailsOpen(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setDetailsOpen(true); } }} role="button" tabIndex={0} aria-label={`פתיחת פרטי ${row.reference_number}`}>
    <div className="flex items-center justify-between gap-2"><span className={`rounded-full px-2 py-0.5 text-[10px] font-extrabold ${row.category === "eco" ? "bg-violet-100 text-violet-700" : row.category === "nonconformity" ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}`}>{labels[row.category]}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${row.status === "open" ? "bg-amber-100 text-amber-800" : row.status === "waiting" ? "bg-sky-100 text-sky-800" : "bg-emerald-100 text-emerald-700"}`}>{statusText(row.status)}</span></div>
    <p className="mt-3 text-xs font-extrabold text-slate-500">{row.reference_number}</p><h3 className="mt-1 line-clamp-2 text-sm font-extrabold leading-5 text-slate-950">{recordTitle(row)}</h3>
    <div className="mt-3 flex items-center justify-between gap-2 text-[11px] text-slate-500"><span className="min-w-0 truncate">{row.opened_by_name || "ללא שם פותח"}</span><span className="shrink-0">{formatDate(row.opened_at)}</span></div>
    <div className="mt-auto flex items-end justify-between gap-2 pt-3"><span className={`h-2 w-2 rounded-full ${overdue ? "bg-red-500" : row.status === "closed" ? "bg-slate-300" : "bg-emerald-500"}`} title={overdue ? "התראה באיחור" : "מצב התראה"} /><span className="text-xs font-bold text-sky-700 group-hover:underline">פתיחת פרטים</span></div>
...
function FollowupsCards({ rows }: { rows: Followup[] }) {
  if (!rows.length) return <div className="rounded-2xl border border-dashed bg-white py-16 text-center text-sm text-slate-500">אין רשומות מתאימות להצגה</div>;
  return <div className="grid grid-cols-[repeat(auto-fill,minmax(205px,1fr))] gap-3">{rows.map((row) => <FollowupCard key={row.id} row={row} />)}</div>;
}
...
export function FollowupsBoard({ rows, activeCategory, statusFilter, query, total, counts }: { rows: Followup[]; activeCategory: Category; statusFilter: StatusFilter; query: string; total: number; counts: Record<Category, number> }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-3 rounded-2xl border bg-white p-1 shadow-sm" role="tablist" aria-label="סוג רשומה">{categories.map((category) => <Link key={category} role="tab" aria-selected={activeCategory === category} prefetch={activeCategory === category ? false : true} href={followupsHref(category, statusFilter, query)} onClick={() => setShowAddForm(false)} className={`flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2.5 text-center text-xs font-extrabold transition-colors sm:block sm:px-2 sm:py-3 sm:text-base ${activeCategory === category ? "bg-slate-950 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100"}`}><span className="truncate">{labels[category]}</span><span className={`rounded-full px-1.5 py-0.5 text-[10px] sm:me-2 sm:px-2 sm:text-xs ${activeCategory === category ? "bg-white/15 text-white" : "bg-amber-100 text-amber-800"}`}>{counts[category]} פעילות</span></Link>)}</div>
    {showAddForm ? <AddFollowupForm key={activeCategory} category={activeCategory} onClose={() => setShowAddForm(false)} /> : null}
    <div className="flex flex-col gap-3 rounded-2xl border bg-white p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between"><form action="/followups" method="get" className="flex w-full gap-2 sm:max-w-md"><input type="hidden" name="category" value={activeCategory} /><input type="hidden" name="status" value={statusFilter} /><div className="relative min-w-0 flex-1"><Search className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input name="q" defaultValue={query} placeholder="חיפוש לפי מספר, שם או הערה" className="h-10 w-full rounded-lg border bg-slate-50 pr-9 pl-3 text-sm" /></div><button className="h-10 rounded-lg bg-slate-950 px-3 text-sm font-bold text-white sm:px-4">חיפוש</button>{query ? <Link href={followupsHref(activeCategory, statusFilter, "")} aria-label="ניקוי חיפוש" title="ניקוי חיפוש" className="flex h-10 w-10 items-center justify-center rounded-lg border text-slate-500"><X className="h-4 w-4" /></Link> : null}</form><div className="grid grid-cols-4 rounded-lg bg-slate-100 p-1 text-xs font-bold sm:text-sm">{(["active", "waiting", "closed", "all"] as StatusFilter[]).map((status) => <Link key={status} href={followupsHref(activeCategory, status, query)} className={`rounded-md px-1.5 py-2 text-center sm:px-3 ${statusFilter === status ? "bg-white text-slate-950 shadow-sm" : "text-slate-500"}`}>{status === "active" ? "פעילות" : status === "waiting" ? "ממתינות" : status === "closed" ? "סגורות" : "הכול"}</Link>)}</div></div>
    <div className="flex flex-wrap items-center justify-between gap-3 px-1 text-sm text-slate-500"><span>{total} רשומות מתאימות</span><div className="flex rounded-lg border bg-white p-1" role="group" aria-label="בחירת תצוגה"><button type="button" onClick={() => setView("table")} aria-pressed={view === "auto" ? undefined : view === "table"} className={`flex h-9 items-center gap-2 rounded-md px-3 font-bold ${view === "table" ? "bg-slate-950 text-white" : view === "auto" ? "text-slate-600 hover:bg-slate-100 md:bg-slate-950 md:text-white" : "text-slate-600 hover:bg-slate-100"}`}><List className="h-4 w-4" />טבלה</button><button type="button" onClick={() => setView("cards")} aria-pressed={view === "auto" ? undefined : view === "cards"} className={`flex h-9 items-center gap-2 rounded-md px-3 font-bold ${view === "cards" ? "bg-slate-950 text-white" : view === "auto" ? "bg-slate-950 text-white md:bg-white md:text-slate-600" : "text-slate-600 hover:bg-slate-100"}`}><LayoutGrid className="h-4 w-4" />כרטיסים</button></div></div>
    <div className={view === "table" ? "block" : view === "cards" ? "hidden" : "hidden md:block"}><FollowupsTable category={activeCategory} rows={rows} /></div>
    <div className={view === "cards" ? "block" : view === "table" ? "hidden" : "md:hidden"}><FollowupsCards rows={rows} /></div>
  </div>;
}
