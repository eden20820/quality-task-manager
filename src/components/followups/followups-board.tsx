...
function FollowupsTable({ category, rows }: { category: Category; rows: Followup[] }) {
  ...
  return <div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="max-h-[65vh] overflow-auto"><table className={`w-full border-collapse text-right text-sm ${category === "eco" || category === "nonconformity" ? "min-w-[1580px]" : "min-w-[1020px]"}`}>
    <thead className="sticky top-0 z-10 bg-slate-100 text-xs text-slate-600 shadow-[0_1px_0_0_#e2e8f0]"><tr><th className="px-4 py-3">מספר</th>{category === "eco" ? <th className="px-4 py-3">פרויקט</th> : null}<th className="px-4 py-3">{category === "eco" ? "תיאור הטופס" : "שם הטופס"}</th>{category === "pka" ? <th className="px-4 py-3">כמות</th> : null}{category === "nonconformity" ? <><th className="px-4 py-3">תלונת ספק</th><th className="px-4 py-3">תלונת לקוח</th><th className="px-4 py-3">יעד אפקטיביות</th><th className="px-4 py-3">אפקטיביות בפועל</th></> : null}<th className="px-4 py-3">שם הפותח</th><th className="px-4 py-3">תאריך הרשומה</th><th className="px-4 py-3">מצב</th>{category === "eco" || category === "nonconformity" ? <th className="px-4 py-3">תאריך סגירה</th> : null}<th className="px-4 py-3">התראה הבאה</th><th className="px-4 py-3">הערות</th><th className="px-4 py-3 text-center">פעולות</th></tr></thead>
    <tbody className="divide-y divide-slate-100">{rows.map((row) => { const alertDate = dueDate(row.created_at); const overdue = row.alerts_enabled && row.status !== "closed" && alertDate < new Date(); return <tr key={row.id} className={`transition-colors hover:bg-slate-50 ${overdue ? "bg-red-50/70" : ""}`}>
      <td className="whitespace-nowrap px-4 py-3 font-extrabold">{row.reference_number}</td>{category === "eco" ? <td className="min-w-44 whitespace-normal px-4 py-3 text-slate-600"><span className="block break-words">{row.eco_project || "—"}</span></td> : null}<td className="min-w-64 whitespace-normal px-4 py-3 font-semibold text-slate-700"><span className="block break-words">{category === "eco" ? row.eco_description || row.name || "—" : row.name || "—"}</span></td>{category === "pka" ? <td className="whitespace-nowrap px-4 py-3">{row.quantity ?? "—"}</td> : null}{category === "nonconformity" ? <><td className="min-w-36 whitespace-normal break-words px-4 py-3">{row.supplier_complaint || "—"}</td><td className="min-w-36 whitespace-normal break-words px-4 py-3">{row.customer_complaint || "—"}</td><td className="min-w-52 whitespace-normal px-4 py-3"><span className="block break-words">{row.effectiveness_due || "—"}</span></td><td className="min-w-52 whitespace-normal px-4 py-3"><span className="block break-words">{row.effectiveness_actual || "—"}</span></td></> : null}<td className="whitespace-nowrap px-4 py-3"><OpenedByEditor row={row} /></td><td className="whitespace-nowrap px-4 py-3 text-slate-600">{formatDate(row.opened_at)}</td>
      <td className="whitespace-nowrap px-4 py-3"><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ${row.status === "open" ? "bg-amber-100 text-amber-800" : row.status === "waiting" ? "bg-sky-100 text-sky-800" : "bg-emerald-100 text-emerald-700"}`}>{row.status === "closed" ? <CheckCircle2 className="h-3.5 w-3.5" /> : <CircleDot className="h-3.5 w-3.5" />}{row.status === "open" ? "פתוחה" : row.status === "waiting" ? "ממתין" : "סגורה"}</span></td>{category === "eco" || category === "nonconformity" ? <td className="whitespace-nowrap px-4 py-3 text-slate-600">{row.closed_at ? formatDate(row.closed_at) : "—"}</td> : null}
...
export function FollowupsBoard({ rows, activeCategory, statusFilter, query, total, counts }: { rows: Followup[]; activeCategory: Category; statusFilter: StatusFilter; query: string; total: number; counts: Record<Category, number> }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="text-3xl font-extrabold sm:text-4xl">פק״ע, אי התאמה, ECO</h1><p className="mt-2 text-slate-500">מעקב מרוכז אחר רשומות פתוחות וסגורות</p></div><div className="flex flex-col gap-2 sm:flex-row">{activeCategory === "eco" ? <EcoImportDialog /> : null}{activeCategory === "nonconformity" ? <NonconformityImportDialog /> : null}<button onClick={() => setShowAddForm((value) => !value)} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 font-bold text-white"><Plus className="h-4 w-4" />הוספת {labels[activeCategory]}</button></div></div>
