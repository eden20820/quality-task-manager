...
function FollowupsTable({ category, rows }: { category: Category; rows: Followup[] }) {
  ...
  return <div className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="max-h-[65vh] overflow-auto"><table className={`w-full border-collapse text-right text-sm ${category === "eco" || category === "nonconformity" ? "min-w-[1580px]" : "min-w-[1020px]"}`}>
    ...
    <tbody className="divide-y divide-slate-100">{rows.map((row) => { const alertDate = dueDate(row.created_at); const overdue = row.alerts_enabled && row.status !== "closed" && alertDate < new Date(); return <tr key={row.id} className={`transition-colors hover:bg-slate-50 ${overdue ? "bg-red-50/70" : ""}`}>
      <td className="whitespace-nowrap px-4 py-3 font-extrabold">{row.reference_number}</td>{category === "eco" ? <td className="max-w-44 px-4 py-3 text-slate-600"><span className="line-clamp-2">{row.eco_project || "—"}</span></td> : null}<td className="max-w-64 px-4 py-3 font-semibold text-slate-700"><span className="line-clamp-2">{category === "eco" ? row.eco_description || row.name || "—" : row.name || "—"}</span></td>{category === "pka" ? <td className="whitespace-nowrap px-4 py-3">{row.quantity ?? "—"}</td> : null}{category === "nonconformity" ? <><td className="px-4 py-3">{row.supplier_complaint || "—"}</td><td className="px-4 py-3">{row.customer_complaint || "—"}</td><td className="max-w-52 px-4 py-3"><span className="line-clamp-2">{row.effectiveness_due || "—"}</span></td><td className="max-w-52 px-4 py-3"><span className="line-clamp-2">{row.effectiveness_actual || "—"}</span></td></> : null}<td className="whitespace-nowrap px-4 py-3"><OpenedByEditor row={row} /></td><td className="whitespace-nowrap px-4 py-3 text-slate-600">{formatDate(row.opened_at)}</td>
