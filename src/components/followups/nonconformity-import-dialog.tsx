...
import { confirmNonconformityImport, previewNonconformityImport } from "@/app/followups/actions";
import type { EcoImportAction, EcoImportResolution } from "@/lib/eco-import";
import type { NonconformityImportPreview } from "@/lib/nonconformity-import";
...
export function NonconformityImportDialog() {
  const inputRef = useRef<HTMLInputElement>(null);
  const [open, setOpen] = useState(false);
  const [preview, setPreview] = useState<NonconformityImportPreview | null>(null);
  ...
  async function readFile(file?: File) {
    ...
    try { const result = await previewNonconformityImport(formData); if (result.success) setPreview(result.preview); else setError(result.message); }
  ...
  function confirm() { if (!preview) return; setError(""); startSaving(async () => { const result = await confirmNonconformityImport(preview.rows, preview.fileName); if (result.success) { setMessage(result.message); setOpen(false); setPreview(null); } else setError(result.message); }); }
  const selected = preview?.rows.filter((row) => (row.action === "new" || row.action === "update") && row.resolution === "import").length ?? 0;

  return <>
    <button type="button" onClick={() => setOpen(true)} className="flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-5 font-bold text-slate-800 shadow-sm hover:bg-slate-50"><Upload className="h-4 w-4" />ייבוא אי התאמות מ-Excel</button>
    ...
    {open ? <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="nonconformity-import-title">
      ...
      <div className="relative z-10 flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b p-5"><div><h2 id="nonconformity-import-title" className="text-2xl font-extrabold">ייבוא מעקב אי התאמות</h2><p className="mt-1 text-sm text-slate-500">כל הלשוניות בקובץ נסרקות ומושוות לרשומות הקיימות לפני שמירה.</p></div><button type="button" onClick={close} disabled={saving} aria-label="סגירה" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><X className="h-5 w-5" /></button></div>
        {!preview ? <div className="overflow-auto p-6"><button type="button" onClick={() => inputRef.current?.click()} disabled={reading} className="flex min-h-56 w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-8 text-center hover:border-slate-400 disabled:opacity-60">{reading ? <LoaderCircle className="mb-3 h-10 w-10 animate-spin" /> : <FileSpreadsheet className="mb-3 h-10 w-10 text-emerald-600" />}<span className="text-lg font-extrabold">{reading ? "סורק ומשווה..." : "בחירת קובץ Excel"}</span><span className="mt-2 text-sm text-slate-500">XLSX או XLS, עד 10MB</span></button><input ref={inputRef} hidden type="file" accept=".xlsx,.xls" onChange={(event) => readFile(event.target.files?.[0])} /></div> : <>
          <div className="border-b bg-slate-50 p-4"><div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{[["חדשים", preview.newCount, "text-emerald-700"], ["עדכונים", preview.updatedCount, "text-amber-700"], ["כפילויות", preview.duplicateCount, "text-violet-700"], ["ללא שינוי", preview.unchangedCount, "text-slate-700"], ["לא תקינים", preview.invalidCount, "text-red-700"]].map(([label, value, tone]) => <div key={String(label)} className={`rounded-xl border bg-white p-3 text-center ${tone}`}><p className="text-xs font-bold">{label}</p><p className="text-2xl font-black">{value}</p></div>)}</div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500"><span>{preview.fileName}{preview.ignoredCount ? ` • ${preview.ignoredCount} שורות שאינן רשומות דולגו` : ""}</span>{preview.updatedCount ? <div className="flex gap-2"><button type="button" onClick={() => applyToUpdates("keep")} className="rounded border bg-white px-2 py-1">שמירת הקיים</button><button type="button" onClick={() => applyToUpdates("import")} className="rounded border bg-white px-2 py-1">עדכון מ-Excel</button><button type="button" onClick={() => applyToUpdates("skip")} className="rounded border bg-white px-2 py-1">דילוג</button></div> : null}</div></div>
          <div className="overflow-auto p-4"><table className="w-full min-w-[920px] text-right text-sm"><thead className="sticky top-0 bg-white text-xs text-slate-500"><tr><th className="p-3">אי התאמה</th><th className="p-3">סיווג</th><th className="p-3">השוואה</th><th className="p-3">פעולה</th></tr></thead><tbody className="divide-y">{preview.rows.map((row, index) => <tr key={row.key} className="align-top"><td className="p-3 font-extrabold">{row.label}<span className="block text-xs font-normal text-slate-400">שורה {row.rowNumber}</span></td><td className="p-3"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${styles[row.action]}`}>{labels[row.action]}</span></td><td className="p-3">{row.error ? <p className="font-semibold text-red-700">{row.error}</p> : row.changes.length ? <div className="space-y-2">{row.changes.map((change) => <div key={change.field} className="grid grid-cols-[120px_1fr_1fr] gap-2 rounded-lg bg-slate-50 p-2"><b>{change.field}</b><span><small className="block">קיים</small>{change.before}</span><span className="text-amber-900"><small className="block">Excel</small>{change.after}</span></div>)}</div> : <span className="text-slate-500">{row.action === "new" ? "רשומה חדשה" : "זהה לרשומה הקיימת"}</span>}</td><td className="p-3">{row.action === "new" || row.action === "update" ? <select value={row.resolution} onChange={(event) => updateRow(index, event.target.value as EcoImportResolution)} className="h-9 rounded-lg border bg-white px-2 font-bold"><option value={row.action === "update" ? "keep" : "import"}>{row.action === "update" ? "שמירת הקיים" : "הוספה"}</option>{row.action === "update" ? <option value="import">עדכון מ-Excel</option> : null}<option value="skip">דילוג</option></select> : <span className="text-xs text-slate-500">לא יישמר</span>}</td></tr>)}</tbody></table></div>
        </>}
